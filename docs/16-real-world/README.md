...
- [Firebase Cloud Messaging (FCM)](fcm.md) — APNs 어댑터, FCM token, Topic 발송, HTTP v1 API, APNs vs FCM 선택
...
- FCM token과 APNs deviceToken은 어떻게 다른가? → [fcm.md](fcm.md)
...
- iOS-only인데 FCM 도입할 가치가 있나? → [fcm.md](fcm.md)
- FCM Notification message vs Data message — iOS에서 동작 차이는? → [fcm.md](fcm.md)
...
- FCM Topic 발송과 token 발송 — 즉시성·확장성 트레이드오프 → [fcm.md](fcm.md)
